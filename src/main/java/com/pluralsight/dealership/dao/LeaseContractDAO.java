...
@Component
public class LeaseContractDAO implements ContractInterface {

    @Override
    public Contract getById(int id) {
        Contract contract = null;
        try (Connection conn = dataSource.getConnection();
             PreparedStatement query = conn.prepareStatement("SELECT * FROM `lease contracts` WHERE id = ?")) {
            query.setInt(1, id);
            ResultSet rows = query.executeQuery();

            if (rows.next()) {
                contract = new Contract();
                contract.setDate(String.valueOf(rows.getInt("date")));
    ...
    public Contract newContract(Contract contract) {
        try (Connection conn = dataSource.getConnection();
             PreparedStatement pstmt = conn.prepareStatement("INSERT INTO `lease contracts`(date,customer,customer_email,vin) VALUES (?,?,?,?)")) {
